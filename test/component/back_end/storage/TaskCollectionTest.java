...
public class TaskCollectionTest {
  ...
  //----------------------------------------------------------------------------------------
    ...
    @Test
    public void Task_entry_in_tree_gets_shifted_when_start_time_changes() throws PrimaryKeyNotFoundException {     
        ...
        // check that the list corresponding to the old start time no longer contains the task
        assertFalse(this.taskCollection.getStartTimeTree().get(this.TASK_1_START).contains(this.task1_));
        
        // check that the list corresponding to the new start time now contains the updated task
        assertTrue(this.taskCollection.getStartTimeTree().get(this.TASK_2_START).contains(newTask));
    }
    
    @Test
    public void Task_entry_in_tree_gets_shifted_when_end_time_changes() throws PrimaryKeyNotFoundException { 
        // check that the list initially contains the old task
        ...
        // check that the list corresponding to the old end time no longer contains the task
        assertFalse(this.taskCollection.getEndTimeTree().get(this.TASK_2_END).contains(this.task2_));
        
        // check that the list corresponding to the new end time now contains the updated task
